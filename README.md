...
- It might work anyway, but to remove the warnings:
  ...
  - In `Edit Configurations`, with `path` being your absolute path to the repository, set:
...
- an `allpix-squared` sub-directory: allpix source code in allpix-squared sub-folder, if installed as described in the readme.md file.
...
- files containing coincident events, which are used as input when running CoReSi (see TODOs)
...
For more information, see the [user manual](doc/readme.md).
...
- show reconstructed source and detector geometry in 3D with
  plot_reco().
  Does not work on MacOS yet, see [napari/opengate conflict](#napariopengate-conflict)
  below.
